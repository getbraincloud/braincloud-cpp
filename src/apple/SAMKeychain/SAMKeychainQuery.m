...
- (void)setPasswordObject:(id<NSCoding>)object {
    ...
    self.passwordData = [NSKeyedArchiver archivedDataWithRootObject:object
                                              requiringSecureCoding:NO
                                                              error:&error];
    if (error) {
        NSLog(@"[SSKeychain] Failed to archive password object: %@", error);
    }
}
...
- (id<NSCoding>)passwordObject {
   ...
   id obj = [NSKeyedUnarchiver unarchivedObjectOfClass:[NSObject class]
                                             fromData:self.passwordData
                                                error:&error];
   if (error) {
       NSLog(@"[SSKeychain] Failed to unarchive password object: %@", error);
   }
   return obj;
}
